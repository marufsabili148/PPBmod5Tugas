...
export default function EditRecipePage({ recipeId, onBack, onSuccess }) {
    ...
    const loadRecipe = async () => {
      try {
        ...
        if (result.success) {
          ...
          // Convert steps to array of strings, ensuring each element is a string
  ...
  const validateForm = () => {
    ...
    // Validate steps
    const validSteps = steps.filter(step => String(step).trim());
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const validIngredients = ingredients.filter(ing => ing.name.trim() && ing.quantity.trim());
      const validSteps = steps.map(step => String(step).trim()).filter(step => step);
